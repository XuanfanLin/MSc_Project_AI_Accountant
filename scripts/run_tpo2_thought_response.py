...
- Single-instruction test: force output blocks <THOUGHT>...</THOUGHT> and <R>...</R>
- Load BASE + LoRA(SFT), merge, then generate
- Few-shot includes key UK tax topics (travel/temporary workplace, Rent-a-Room choice, director loan write-off)
- Decode only newly generated tokens to avoid prompt echo contaminating parsing; parser has fallbacks
- Output ONLY a single JSON line with: {"query": ..., "thought": ..., "response": ...}
"""
...
# ===== Tags (thought & final answer) =====
TH_BEGIN, TH_END = "<THOUGHT>", "</THOUGHT>"
RESP_BEGIN, RESP_END = "<R>", "</R>"

# ===== Llama 3 chat template (fallback to base) =====
...
# ===== Custom stopping criteria: stop when a stop string is seen =====
...
# ===== Few-shot aligned with AI Accountant style (compliant, concise, key points) =====
def build_messages(user_query: str) -> List[dict]:
    ...
    # --- Few-shot 1: Travel expenses (revised) ---
    ...
    # --- Few-shot 2: Rent-a-Room (revised) ---
    ...
    # --- Few-shot 3: Director’s loan write-off (revised) ---
...
# ===== Parse output blocks (with fallbacks) =====
def parse_blocks(text: str) -> Tuple[str, str]:
    ...
    if not final:
        # Fallback: capture from <R> until end-of-turn tokens if </R> is missing
...
def main():
    # Allow overriding the test question via environment variable QUERY
    ...
    base = AutoModelForCausalLM.from_pretrained(
        BASE_MODEL_PATH,
        torch_dtype=DTYPE,
        attn_implementation="eager",  # Switch to "flash_attention_2" if your environment supports it
        device_map=None
    ).to(device).eval()
    ...
    # Decode only new tokens (avoid few-shot echo contaminating parsing)
    ...
    # Consider only the newly generated part
    new_tokens = outputs[0, input_len:]
    raw = tok.decode(new_tokens, skip_special_tokens=False)

    thought, final = parse_blocks(raw)

    # --- Only output the parsed result as a single JSON line ---
    print(json.dumps({
        "query": instruction,
        "thought": thought or "",
        "response": final or ""
