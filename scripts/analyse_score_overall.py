...
if not label_to_file:
    raise SystemExit(f"在 {OUT_DIR} 未发现任何 *_scores.jsonl 文件。")

frames = []
for label, path in label_to_file.items():
    part = load_scores_from_file(path, label)
    if part.empty:
        print(f"⚠️  {label}: {path} 无有效分数，跳过。")
        continue
    frames.append(part)
if not frames:
    raise SystemExit("未能从任何文件解析到有效分数。")
...
sns.boxplot(
    ...
    medianprops=dict(color="black", lw=2)  # 黑线=median
)
