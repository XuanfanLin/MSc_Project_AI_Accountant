...
# ===== Environment =====
os.environ.setdefault("CUDA_VISIBLE_DEVICES", "0")                 # Change GPU index if needed
...
ADAPTER_PATH    = "/home/zceexl3/ai_accountant/models/dpo_1/checkpoint-500"
DPO_DATA_PATH   = "/home/zceexl3/ai_accountant/scripts/tpo2/output/dpo_pairs_gpt_enhanced.jsonl"
DPO_OUTPUT_DIR  = "/home/zceexl3/ai_accountant/models/tpo_2"

# Reference model optional (rDPO saves memory if True)
...
def ensure_plain_prompt(text: str) -> str:
    """If 'instruction' is missing, fall back to an existing prompt; add the 'Response' prefix if absent."""
    text = (text or "").strip()
...
def load_preference_dataset(path: str) -> Dataset:
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # Use 'prompt' only as a fallback when 'instruction' is missing
...
def main():
    ...
    base.config.use_cache = False  # save memory during training
    ...
    cfg = DPOConfig(
        ...
        save_strategy="steps",
        save_steps=100,
        eval_strategy="steps",   # parameter name as used here
        eval_steps=50,
        bf16=True,
        beta=0.1,
    ...
    # Trainer (compatibility with different TRL versions)
